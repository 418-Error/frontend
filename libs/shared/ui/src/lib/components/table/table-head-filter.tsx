import { Dispatch, type MouseEvent, SetStateAction, useEffect, useState } from 'react'
...
import { upperCaseFirstLetter } from '@draco/utils'
  ...
  return (
    <div className="flex items-center">
      ...
      {hasFilter && (
        <span
          role="button"
          className="flex items-center px-2 relative -left-7 text-neutral-50 text-xs cursor-pointer"
          onClick={(event) => cleanFilter(event)}
        >
          <Icon name={IconAwesomeEnum.XMARK} className='w-5 ' />
        </span>
      )}
    </div>
  )
